...
const LaporanPenghapusanB = () => {
  ...
  const fetchData = async () => {
    const response = await axios
      .get(`http://localhost:8000/api/kibb/all/verifikasi/${location.state}`)
  ...
  const openDetails = (data) => {
    navigate(`/admin/verifikasi/detail/kib-b/${data.id_usulan_b}`, {
      state: data,
    });
  };
          ...
          <div className="flex flex-col lg:ml-64 pt-[8.7rem] px-5 w-auto">
            <div className="block p-6 bg-white border border-gray-200 rounded-lg shadow">
              <h5 class="mb-5 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                Pelaporan Penghapusan / KIB B
              </h5>
              <a
                href={`http://localhost:8000/api/kibb/laporan-penghapusan/${location.state}`}
                download
