...
const LaporanPenghapusanE = () => {
  ...
  const generateExcel = async () => {
    const response = await axios
      .get(`http://localhost:8000/api/kibe/all/verifikasi/excel/${location.state}`)
      .catch((err) => console.log(err));
    if (response) {
      console.log("response: ", response);
      const data = response.data;
          ...
          <div className="flex flex-col lg:ml-64 pt-[8.7rem] px-5 w-auto">
            <div className="block p-6 bg-white border border-gray-200 rounded-lg shadow">
              <h5 class="mb-5 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                Pelaporan Penghapusan / KIB E
              </h5>
              <a
                href={`http://localhost:8000/api/kibe/laporan-penghapusan/${location.state}`}
                download
