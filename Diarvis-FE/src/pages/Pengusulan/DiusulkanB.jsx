...
const DiusulkanB = () => {
  ...
  const openDetails = (data) => {
    navigate(`/diusulkan/kib-b/detail/${data.id_usulan_b}`, {
      state: data,
    });
  };

  const handleEdit = (data) => {
    navigate(`/diusulkan/kib-b/edit/${data.id_usulan_b}`, {
      state: data,
    });
  };
    ...
    try {
      await Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete(`http://localhost:8000/api/kibb/usulan/${id}`);
          Swal.fire({
            icon: "success",
        ...
        Cell: ({ row }) => (
          <div className="flex justify-center ml-2">
            ...
            <button
              ...
              onClick={() => handleDelete(row.original.id_usulan_b)}
            >
              <AiFillDelete />
            </button>
          </div>
