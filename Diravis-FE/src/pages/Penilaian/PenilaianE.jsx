...
const PenilaianE = () => {
  ...
  const fetchData = async () => {
    const response = await axios
      .get(`http://localhost:8000/api/kibe/penilaian`)
  ...
  const openDetails = (data) => {
    navigate(`/penilaian/kib-e/detail/${data.id_usulan_e}`, { state: data });
  };
  ...
  const handleSubmitPenilaian = async (e) => {
    ...
    {statusPenilaian === false
      ? setLinkPostPenilaian("http://localhost:8000/api/kibe/penilaian/ditolak")
      : setLinkPostPenilaian(
          "http://localhost:8000/api/kibe/penilaian/diterima"
        );}
    try {
      const config = {
        headers: {
        ...
        Cell: ({ row }) => (
          <div className="flex justify-center ml-2">
            ...
            <button
              ...
              onClick={() => handleInputPenilaian(row.original.id_usulan_e)}
            >
