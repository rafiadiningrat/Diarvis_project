...
const VerifikasiE = () => {
  ...
  const handleRadioDiterima = (e) => {
    ...
    setLinkPostVerifikasi("http://localhost:8000/api/kibe/verifikasi/diterima");
  };

  const handleRadioDitolak = (e) => {
    setStatusVerifikasi(e.target.value === "false");
    setLinkPostVerifikasi("http://localhost:8000/api/kibe/verifikasi/ditolak");
  };

  const fetchData = async () => {
    const response = await axios
      .get(`http://localhost:8000/api/kibe/verifikasi/${codeFilterUpb}`)
  ...
  const openDetails = (data) => {
    navigate(`/verifikasi/kib-e/detail/${data.id_usulan_e}`, { state: data });
  };
        ...
        Cell: ({ row }) => (
          <div className="flex justify-center ml-2">
            ...
            <button
              ...
              onClick={() => handleInputVerifikasi(row.original.id_usulan_e)}
            >
